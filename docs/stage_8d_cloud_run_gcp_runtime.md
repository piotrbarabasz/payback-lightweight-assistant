...
gcloud run services update "$SERVICE_NAME" \
  --region="$GCP_REGION" \
  --service-account="$CLOUD_RUN_SERVICE_ACCOUNT" \
  --update-env-vars="INTENT_BACKEND=vertex_llm,GCP_PROJECT_ID=$GCP_PROJECT_ID,VERTEX_AI_LOCATION=$VERTEX_AI_LOCATION,VERTEX_INTENT_MODEL=${VERTEX_INTENT_MODEL:-gemini-2.5-flash},INTENT_LLM_TIMEOUT_SECONDS=${INTENT_LLM_TIMEOUT_SECONDS:-3}" \
  --project="$GCP_PROJECT_ID"
```
