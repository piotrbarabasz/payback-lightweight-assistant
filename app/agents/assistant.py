...
class AssistantAgent:
    """Deterministic assistant orchestration agent.

    This agent coordinates intent results, decision policy, product retrieval,
    and response assembly. It is deliberately single-pass and deterministic by
    default; it is not an autonomous planning or LLM tool-use loop.
    """
    ...
    def __init__(
        ...
        *,
        ...
        product_loader: Callable[[], list[Product]] = load_products,
        retriever_factory: Callable[[], ProductRetriever] = get_product_retriever,
    ) -> None:
    ...
    def respond(
        ...
        if self._decision_agent.should_return_without_retrieval(intent_result):
            return _response_from_intent_result(
                self._decision_agent.prepare_no_retrieval_result(intent_result),
                results=[],
            )

        products = self._product_loader()
        retriever = self._retriever_factory()
        is_comparison = (
            intent_result.next_best_action == NextBestAction.COMPARE_PRODUCTS
        )
        ...
        if not results:
            return _response_from_intent_result(
                self._decision_agent.with_clarifying_action(intent_result),
                results=[],
            )
...
AssistantOrchestratorAgent = AssistantAgent
